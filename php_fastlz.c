...
PHP_FASTLZ_API int fastlz_xdecompress(char *compressed, int compressed_len, char** uvalue TSRMLS_DC)
{
	...
	if (compressed_len > sizeof(uint32_t)) {
		...
		if ((compressed_len - sizeof(uint32_t)) == value_len) {
			char *value;
			...
			php_error_docref(NULL TSRMLS_CC, E_WARNING, "Compressed data size mismatch: header says %u bytes but I get %u bytes. Unable to decompress data", value_len, (compressed_len - sizeof(uint32_t)));
			return 0;
		}
	}
